...
export class InwardFormUtils {
  ...
  static async resolveSkuId(article: any, company: Company): Promise<number> {
    ...
    try {
      ...
      const sku = Number(res?.sku_id ?? res?.id ?? res?.ID ?? res?.SKU_ID)
      ...
      throw new Error(
        `Could not resolve SKU ID for "${article.item_description}": ${error instanceof Error ? error.message : String(error)}`,
      )
    }
  }
  ...
  static validateFormData(formData: any, articles: any[], boxes: any[]): ValidationResult {
    ...
    if (boxes.length > 0) {
      ...
      if (Math.abs(boxGrossTotal - articleGrossTotal) > 0.01) {
        ...
        additionalFieldErrors["box_gross_weight_total"] =
          "Box gross weight total does not match article gross weight total"
      }
    }
   ...
   */
  static buildSubmissionPayload(formData: any, articles: any[], boxes: any[], company: Company): InwardFormData {
    return {
      company,
      transaction: {
        ...formData,
      ...
      if (article.quantity_units > 0 && (article.uom === "BOX" || article.uom === "CARTON")) {
        ...
        for (let i = 0; i < article.quantity_units; i++) {
          ...
          const existingBox = existingBoxes.find(
            (box) =>
              box.box_number === boxCounter && box.article === (article.item_description || `Article ${article.id}`),
          )
          ...
          newBoxes.push({
            ...
            net_weight: existingBox
              ? existingBox.net_weight
              : article.net_weight > 0
                ? article.net_weight / article.quantity_units
                : 0,
            gross_weight: existingBox
              ? existingBox.gross_weight
              : article.total_weight > 0
                ? article.total_weight / article.quantity_units
                : 0,
            lot_number: article.lot_number || "",
          })
          boxCounter++
  ...
  static updateBox(boxId: string, field: string, value: any, boxes: any[]): any[] {
    return boxes.map((box) => (box.id === boxId ? { ...box, [field]: value } : box))
  }
  ...
  static async handlePrintBox(
    ...
    try {
      ...
      console.log("[v0] Looking for article matching box:", box.article)
      console.log(
        "[v0] Available articles:",
        articles.map((a) => a.item_description),
      )
      ...
      if (!associatedArticle) {
        console.error("[v0] Article lookup failed:", {
          boxArticle: box.article,
          availableArticles: articles.map((a) => a.item_description),
        })
        throw new Error(
          `Could not find associated article "${box.article}" for box ${box.box_number}. Available articles: ${articles.map((a) => a.item_description).join(", ")}`,
        )
      }

      console.log("[v0] Found article:", associatedArticle)
      ...
      console.log("[v0] Printing QR label for box:", box.box_number, "to printer:", selectedPrinter)
      console.log("[v0] QR Payload:", qrPayload)
      ...
      console.error("[v0] Error in handlePrintBox:", error)
      return {
        success: false,
        error: error instanceof Error ? error.message : String(error),
      }
   ...
   */
  static async generateQRPreview(box: any, formData: any, articles: any[], company: Company): Promise<string> {
    try {
      console.log("[v0] Generating QR preview for box:", box.article)
      console.log(
        "[v0] Available articles:",
        articles.map((a) => a.item_description),
      )
      ...
      const associatedArticle = articles.find((art) => art.item_description === box.article)
      if (!associatedArticle) {
        console.error("[v0] Article lookup failed in preview:", {
          boxArticle: box.article,
          availableArticles: articles.map((a) => a.item_description),
        })
        throw new Error(`Could not find associated article "${box.article}" for box ${box.box_number}`)
      }

      console.log("[v0] Found article for preview:", associatedArticle)
      ...
      console.error("[v0] Error generating QR preview:", error)
      throw error
    }
  }
}
