...
export interface DropdownResponse {
  ...
  options: {
    ...
    item_ids?: number[] // Add item_ids to track the actual database IDs
  }
  meta: {
...
export const authApi = {
  ...
  async fetchDashboardStats(companyCode: string): Promise<{
    ...
    try {
      const apiUrl = `${process.env.NEXT_PUBLIC_API_URL ?? 'http://localhost:8000'}/dashboard/${companyCode}/stats`
      ...
      if (response.ok) {
        ...
        return {
          totalInward: data.total_inward || 0,
          totalTransfers: data.total_transfers || 0,
          pendingLabels: data.pending_labels || 0,
          activeItems: data.active_items || 0,
          weeklyInwardChange: data.weekly_inward_change || 0,
          weeklyTransferChange: data.weekly_transfer_change || 0
        }
      }
  ...
  async fetchRecentInward(companyCode: string, limit: number = 5): Promise<any[]> {
    try {
      ...
      return (data.records || []).map((record: any) => ({
        ...
        vendor_supplier_name: 'N/A', // This might need to be fetched from detail
        quantity_units: record.quantities_and_uoms?.[0]?.split(' ')?.[0] || '0',
        uom: record.quantities_and_uoms?.[0]?.split(' ')?.[1] || 'pcs',
        status: 'Completed', // Default status since we don't have this in list
        transaction_type: 'Purchase', // Default since we don't have this in list
        transaction_no: record.transaction_id,
  ...
  async fetchRecentTransfers(companyCode: string, limit: number = 5): Promise<any[]> {
    try {
      const apiUrl = `${process.env.NEXT_PUBLIC_API_URL ?? 'http://localhost:8000'}/transfer/${companyCode}?per_page=${limit}&page=1`
...
export const dropdownApi = {
  async fetchDropdown(params: {
    ...
    const { company, item_category, sub_category, search, limit, offset } = params
    ...
    try {
      ...
      const apiUrl = `${process.env.NEXT_PUBLIC_API_URL ?? 'http://localhost:8000'}/api/sku/dropdown?${query.toString()}`
      ...
      return {
        company,
        selected: { item_description: null, item_category: null, sub_category: null },
        auto_selection: {
          resolved_from_item: { item_category: null, sub_category: null },
          resolved_from_category_sub: { item_descriptions: [] }
        },
        options: { item_descriptions: [], item_categories: [], sub_categories: [] },
        meta: {
  ...
  async fetchSkuId(params: {
    ...
    item_category?: string
    sub_category?: string
  }): Promise<{ id?: number; sku_id?: number; ID?: number; SKU_ID?: number }> {
    const { company, item_description, item_category, sub_category } = params
    
    console.log("=== FETCHING SKU ID FROM REAL API ===")
    console.log("Fetching SKU ID for:", { company, item_description, item_category, sub_category })
    ...
    try {
      ...
      const apiUrl = `${process.env.NEXT_PUBLIC_API_URL ?? 'http://localhost:8000'}/api/sku/id?${query.toString()}`
      ...
      // The API returns {id, item_description, item_category, sub_category, company}
      // We need to return it in the format expected by the calling code
      return { 
        sku_id: data.id,
        id: data.id,
        ID: data.id,
