  ...
  persist(
    (set, get) => ({
        ...
        try {
          ...
          if (!response.ok) {
            ...
            try {
              ...
              if (response.status === 401) {
                ...
                errorMessage =
                  typeof errorData.detail === "string" ? errorData.detail : "Login failed. Please try again."
          ...
          const user: User = {
            ...
            companies: userData.companies
              ? userData.companies.map((comp: any) => ({
      ...
      setCurrentCompany: async (company: Company) => {
        ...
        try {
          ...
          if (!response.ok) {
            ...
            set({ isLoading: false })
            throw new Error(`Failed to access ${company}: ${response.status}`)
          }
      ...
      hasPermission: (module: Module, action: Action): boolean => {
        ...
        if (!currentCompanyAccess.modules || !Array.isArray(currentCompanyAccess.modules)) {
          console.warn("[AUTH] No modules array in current company access")
          return false
        }
      ...
      hasCompanyAccess: (company: Company): boolean => {
        const { user } = get()
        if (!user || !user.companies || !Array.isArray(user.companies)) return false
